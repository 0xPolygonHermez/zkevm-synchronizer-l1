...
func TestAddSequence(t *testing.T) {
	...
	seq := &pgstorage.SequencedBatches{
		...
		L1BlockNumber:   123,
		ForkID:          123,
		Timestamp:       time.Date(2023, 12, 14, 14, 30, 45, 0, time.Local).UTC(),
		ReceivedAt:      time.Date(2024, 1, 14, 14, 30, 45, 0, time.Local).UTC(),
		Source:          "test"}
...
func populateSequences(t *testing.T, ctx context.Context, l1BlockNumber uint64, num_seq int, storage *sqlstorage.SqlStorage, dbTx entities.Tx) {
	if num_seq == 0 {
		return
	}
	seq := &pgstorage.SequencedBatches{
