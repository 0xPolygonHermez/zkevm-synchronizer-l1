...
func (p *PostgresStorage) AddBlock(ctx context.Context, block *L1Block, dbTx pgx.Tx) error {
	const addBlockSQL = "INSERT INTO sync.block (block_num, block_hash, parent_hash, received_at, sync_version) VALUES ($1, $2, $3, $4, $5)"

	e := p.getExecQuerier(dbTx)
	_, err := e.Exec(ctx, addBlockSQL, block.BlockNumber, block.BlockHash.String(), block.ParentHash.String(), block.ReceivedAt, block.SyncVersion)
	return err
}

// GetLastBlock returns the last L1 block.
func (p *PostgresStorage) GetLastBlock(ctx context.Context, dbTx pgx.Tx) (*L1Block, error) {
	const getLastBlockSQL = "SELECT block_num, block_hash, parent_hash, received_at,sync_version FROM sync.block ORDER BY block_num DESC LIMIT 1"
...
func (p *PostgresStorage) GetBlockByNumber(ctx context.Context, blockNumber uint64, dbTx pgx.Tx) (*L1Block, error) {
	const getBlockByNumberSQL = "SELECT block_num, block_hash, parent_hash, received_at,sync_version FROM sync.block WHERE block_num = $1"
...
func (p *PostgresStorage) GetPreviousBlock(ctx context.Context, offset uint64, dbTx pgx.Tx) (*L1Block, error) {
	const getPreviousBlockSQL = "SELECT block_num, block_hash, parent_hash, received_at,sync_version FROM sync.block ORDER BY block_num DESC LIMIT 1 OFFSET $1"
...
func scanBlock(row pgx.Row) (L1Block, error) {
	...
	block := L1Block{}
	if err := row.Scan(&block.BlockNumber, &blockHash, &parentHash, &block.ReceivedAt, &block.SyncVersion); err != nil {
		return block, err
	}
