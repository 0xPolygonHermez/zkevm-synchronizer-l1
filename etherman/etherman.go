...
func NewClient(cfg Config) (*Client, error) {
	...
	polygonZkEVMGlobalExitRoot, err := polygonzkevmglobalexitroot.NewPolygonzkevmglobalexitroot(cfg.PolygonZkEVMGlobalExitRootAddress, ethClient)
	if err != nil {
		return nil, err
	}
	polygonRollupManager, err := polygonrollupmanager.NewPolygonrollupmanager(cfg.PolygonRollupManagerAddress, ethClient)
	if err != nil {
		return nil, err
	}
	// Get RollupID
	rollupID, err := polygonRollupManager.RollupAddressToID(&bind.CallOpts{Pending: false}, cfg.PolygonZkEvmAddress)
	if err != nil {
		return nil, err
	}
	log.Debug("rollupID: ", rollupID)
	var scAddresses []common.Address
	scAddresses = append(scAddresses, cfg.PolygonZkEVMGlobalExitRootAddress, cfg.PolygonRollupManagerAddress)
...
func (etherMan *Client) processEvent(ctx context.Context, vLog types.Log, blocks *[]Block, blocksOrder *map[common.Hash][]Order) error {
	...
	case updateGlobalExitRootSignatureHash:
		log.Fatal("updateGlobalExitRootSignatureHash event detected. Ignoring...NOT SUPPORTED!")
	...
	case depositEventSignatureHash:
		log.Debug("depositEventSignatureHash event detected. Ignoring...")
		return nil
	case claimEventSignatureHash:
		log.Debug("claimEventSignatureHash event detected. Ignoring...")
		return nil
	case oldClaimEventSignatureHash:
		log.Debug("oldClaimEventSignatureHash event detected. Ignoring...")
		return nil
	case newWrappedTokenEventSignatureHash:
		log.Debug("newWrappedTokenEventSignatureHash event detected. Ignoring...")
		return nil
